...
extern
 "C" {
    ...
    int
     fe_timeout_add(int interval, void *func, void *data) {
    ...
    int
     fe_args(int argc, char *argv[]) {
     	     KAboutData *about =
	    new KAboutData("vertigo", "Vertigo", VERSION, "An IRC Client for KDE",
	...
	if (prefs.wait_on_exit) {
	    ;
	    XChatCleanupHandler *h = new XChatCleanupHandler();
	    h->cleanup();
    ...
    void
     fe_add_chan_list(struct server *serv, char *chan, char *users,
		      char *topic) {
    ...
    void
     fe_add_ban_list(struct session *sess, char *chan, char *users,
		     char *topic) {
    ...
    void
     fe_userlist_insert(struct session *sess, struct User *newuser,
			int row, int sel) {
	...
	XChatServerlist *l = xchatapp->serverList();
